...
export default {
  ...
  computed: {
    cfLengthOfDataArray() {
      const arFromOrm = clientSideTable.fnGetValidUniqueUuidRows()
    ...
    cfArOfPhoneNumberForDisplayInTable() {
      ...
      const arFromOrm = clientSideTable.fnGetValidUniqueUuidNotEmptyRows('countryCode')
      ...
      */
      ...
      if (arFromOrm.length) {
        ...
        for (let i = startDataRowInidex; i < arFromOrm.length && i < endDataRowIndex; i++) {
          ...
          // For date format ref: /cts/pt-info/single/1time-Mrow-1Field/phoneNumber/view-layer/timeline-ct.vue:53
