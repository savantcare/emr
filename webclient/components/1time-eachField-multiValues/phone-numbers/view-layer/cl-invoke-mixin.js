...
export default {
  methods: {
    ...
    mxOpenEditCtInEditLayer(pClientDataRowId) {
      ...
      */
      const payload = { searchTerm: 'edit phone number', pPropsToGiveToCt: pClientDataRowId }
      this.$store.commit('mtfShowNewFirstTabInEditLayerFromSearchPhrase', payload)
    },
    mxOpenDPrompt(pClientDataRowId) {
      ...
      this.$prompt(arResultsFromOrm.description, 'Delete phone number', {
        ...
        .then(async ({ value }) => {
          const status = await clientTbl.fnSendDeleteDataToServer(pClientDataRowId, arResultsFromOrm.uuid, value)
          if (status === 1) {
            this.$message({
              type: 'success',
