  ...
  <div>
    <el-card id="thresolds" :style="status || this.features.showDetails ? '' : 'display:none'"
      ><div :style="status ? '' : 'text-decoration: line-through dotted red'">
        90834<span v-if="status">&check;</span>
      </div>

      <div :style="statusC1 ? '' : 'text-decoration: line-through dotted red'">
        1. Billing duration > 37 and less then 53 <span v-if="statusC1">&check;</span>
      </div>
...
export default {
  ...
  computed: {
    statusC1() {
      return this.features.billingDuration > 37 && this.features.billingDuration < 53
    },
    statusC2() {
