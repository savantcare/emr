  ...
  methods: {
    ...
    scrollHandler() {
      ...
      this.scrollTimer = setTimeout(
        function () {
          const items = this.items.map((item, index) => Math.abs(this.$refs.item[index].getBoundingClientRect().left))
...
.vhl-list {
  display: flex;
  padding-bottom: 10px; /* If i make this +8/-8 then I can see the scroll bar at the bottom.*/
  margin-bottom: -10px;
