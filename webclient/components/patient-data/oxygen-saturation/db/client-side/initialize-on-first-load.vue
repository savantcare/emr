...
<script>
import initializeOxygenSaturationComponent from '@/components/patient-data/oxygen-saturation/db/client-side/static-data/insert-into-master-of-search-phrases-ct.vue'
...
export default {
  ...
  async mounted() {
    ...
    if (process.env.loadInitialDataFromServer === true) {
      const proOxygenSaturationFromDB = await clientTbl.api().get(clientTbl.apiUrl+'/'+ptUuidFromOrm.fieldValue)
      if (proOxygenSaturationFromDB.ok) {
      }
    }
  },
}
