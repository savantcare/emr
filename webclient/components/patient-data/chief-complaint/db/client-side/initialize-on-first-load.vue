...
export default {
  ...
  async mounted() {
    /*
        ...
        const proChiefComplaintFromDB = await clientTbl.api().get(clientTbl.apiUrl + '/getAll')
      */
    if (process.env.loadInitialDataFromServer === true) {
      ...
      const proChiefComplaintFromDB = await clientTbl.api().get(clientTbl.apiUrl+'/'+ptUuidFromOrm.fieldValue)
      if (proChiefComplaintFromDB.ok) {
      }
    }
  },
}
