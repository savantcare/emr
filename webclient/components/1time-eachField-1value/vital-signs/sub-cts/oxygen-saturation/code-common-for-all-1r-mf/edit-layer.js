...
export default {
    ...
    cfTimeLineDataAr() {
      ...
      if (arFromClientSideTable.length) {
        ...
        for (let i = 0; i < arFromClientSideTable.length; i++) {
          rowInTimeLine = {}
          rowInTimeLine.oxygenSaturationInSpo2 = arFromClientSideTable[i].oxygenSaturationInSpo2
  ...
  methods: {
    async mfOnReviewed() {
      ...
      const rowToUpsert = clientTbl.find(this.dnClientSideIdOfCopiedRowBeingChanged)
    ...
    mfGetCopiedRowBeingChangedFldVal(pFldName) {
      ...
      */
      ...
      const value = clientTbl.fnGetFldValue(this.dnClientSideIdOfCopiedRowBeingChanged, pFldName)
      return value
    },
