    ...
    async MsgFromSktForRemToAdd(pData) {
      ...
      if (
        ...
        const arFromClientSideTable = await clientSideTable.insert({
          data: {
            vnRowStateInSession: 9, // For meaning of diff values read webclient/cts/others/crud/forms.md
  ...
  methods: {
    fnSetRowStatus(pClientSidePrimaryKeyValue) {
      clientSideTable.update({
        ...
        data: {
          vnRowStateInSession: 1, // For meaning of diff values read webclient/cts/others/crud/forms.md
        },
      })
    },
  },
