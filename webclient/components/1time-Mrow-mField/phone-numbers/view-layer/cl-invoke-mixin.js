...
import clientTbl from '~/components/1time-eachField-multiValues/reminders/db/client-side/structure/reminders-of-a-patient-table.js'
...
export default {
  methods: {
    ...
    mxOpenEditCtInEditLayer(pClientSideDataRowId) {
      ...
      */
      const payload = { searchTerm: 'edit phone number', pPropsToGiveToCt: pClientSideDataRowId }
      this.$store.commit('mtfShowNewFirstTabInEditLayerFromSearchPhrase', payload)
    },
    mxOpenDPrompt(pClientSideDataRowId) {
      ...
      this.$prompt(arResultsFromOrm.description, 'Delete phone number', {
        ...
        .then(async ({ value }) => {
          const status = await clientTbl.fnSendDeleteDataToServer(pClientSideDataRowId, arResultsFromOrm.uuid, value)
          if (status === 1) {
            this.$message({
              type: 'success',
